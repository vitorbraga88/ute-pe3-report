...
UTE_PE3.Camera = {
  ...
  /** Open file picker for camera/gallery */
  async capture() {
  ...
  async capturePTS() {
    try {
      this.ptsPhoto = await this.capture();
      this.renderPTSPhoto();
    } catch (e) {
  ...
  renderPhotos() {
    ...
    // Add "add photo" button card
    grid.insertAdjacentHTML('beforeend', `
      <div class="photo-card" style="display:flex;align-items:center;justify-content:center;cursor:pointer;min-height:100px;border:2px dashed var(--gray-300)"
           onclick="UTE_PE3.Camera.addPhoto()">
        <div style="text-align:center;color:var(--gray-400)">
          <div style="font-size:2rem">+</div>
          <div style="font-size:0.75rem">Adicionar Foto</div>
        </div>
      </div>
    `);
  },
  ...
  renderPTSPhoto() {
    ...
    if (this.ptsPhoto) {
      container.innerHTML = `
        <div class="photo-card" style="max-width:200px">
      ...
      container.innerHTML = `
        <button class="btn btn-secondary btn-sm" onclick="UTE_PE3.Camera.capturePTS()">
          📷 Fotografar PTS
        </button>
      `;
    }
